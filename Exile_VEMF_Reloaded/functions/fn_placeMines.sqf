...
_ok = false;
_enabled = ([["DynamicLocationInvasion"],["placeMines"]] call VEMFr_fnc_getSetting) select 0;
    ...
    if (count _pos isEqualTo 3) then
    {
        ...
        if (_min > -1) then
        {
            ...
            if (_max > _min) then
            {
                ...
                if (_amount > -1) then
                {
                    _minesMode = [[["DynamicLocationInvasion"],["minesMode"]] call VEMFr_fnc_getSetting, 0, 1, [0]] call BIS_fnc_param;
